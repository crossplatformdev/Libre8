...
fn display_memory(_window: &mut Window, memory: &[u8], page_base: usize, current_addr: usize) {
    let width = 16 * 32 + 128;
    let height = 16 * 20 + 20;
    unsafe {
        if draw_rect {
    ...
    draw_text(mem_display_start_x + 90, mem_display_start_y, &format!("CUR {:04X}", current_addr.min(memory.len().saturating_sub(1))), 0xFFFF00, COL_CARD);

    for i in 0..16 {
        let y = mem_display_start_y + (i * 20) + 20;
        for j in 0..16 {
            let cell_addr = page_base + i * 16 + j;
            if cell_addr >= memory.len() {
                continue;
            }
            ...
            let value_str = format!("{:02X}", memory[cell_addr]);
...
// Update signature: add current_addr
fn display_vram(_window: &mut Window, vram: &[u8], page_base: usize, current_addr: usize) {
    let width = 16 * 32 + 128;
    let height = 16 * 20 + 20;
    unsafe {
        if draw_rect {
    ...
    draw_text(vram_display_start_x + 70, vram_display_start_y, &format!("CUR {:04X}", current_addr.min(vram.len().saturating_sub(1))), 0xFFFF00, COL_CARD);

    for i in 0..16 {
        let y = vram_display_start_y + (i * 20) + 20;
        for j in 0..16 {
            let cell_addr = page_base + i * 16 + j;
            if cell_addr >= vram.len() {
                continue;
            }
            ...
            let value = format!("{:02X}", vram[cell_addr]);
            if cell_addr == current_addr {
...
fn refresh_update(
    ...
    refresh(window, bytes, opcode, addr, x, y, video_buffer);
...
fn refresh(window: &mut Window, bytes: &[u8], opcode: u8, addr: usize, x: usize, y: usize, video_buffer: &[u8]) {
    ...
    unsafe {
        ...
        if mem_page != LAST_MEM_PAGE_BASE || draw_rect {
            display_memory(window, bytes, mem_page, addr.min(bytes.len().saturating_sub(1)));
        ...
        if vram_page != LAST_VRAM_PAGE_BASE || draw_rect {
            display_vram(window, video_buffer, vram_page, cur_px);
    ...
    while addr < num_bytes {
        ...
        match opcode {
            ...
            OUTA | OUTB | OUTC | OUTD => {
                ...
                addr += 1;
                // refresh+update on every addr increment (non-POKE)
                refresh_update(&mut window, &bytes, opcode, addr, x, y, &video_buffer);
            }
            ...
            JMP => {
                addr = read_offset(prev_addr, &bytes);
                log(
        ...
        unsafe {
            ...
            // Update display:
            // - POKE: throttle updates as before
            // - Non-POKE: we already refreshed+updated on each addr increment; just push HZ text if drawn
            if opcode == POKE {
                // Only update with throttle for POKE
                if addr % (WIDTH * HEIGHT * video_speed_factor) == 0 {
                    refresh(&mut window, &bytes, opcode, addr, x, y, &video_buffer);
                    update_window(&mut window);
                }
            } else {
                // No extra refresh; just a tiny presentational push so HZ text shows promptly
                update_window(&mut window);
