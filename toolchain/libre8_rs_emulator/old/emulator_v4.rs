...
// Helper: compute 16x16-page base for a given offset
#[inline]
fn page_base_256(offset: usize) -> usize {
    offset & !(CARD_PAGE_SIZE - 1)
}
...
fn refresh(window: &mut Window, bytes: &[u8], opcode: u8, addr: usize, x: usize, y: usize, video_buffer: &[u8]) {
    unsafe {
        ...
        if mem_page != LAST_MEM_PAGE_BASE {
        ...
        if vram_page != LAST_VRAM_PAGE_BASE {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while addr < num_bytes {
        ...
        match opcode {
            POKE => {
                // Blit up to 63 pixels (2x2 scaled) with bounds checks
                for i in 1..64 {
                    let p = addr + i;
                    if p >= num_bytes {
                        break;
                    ...
                    if x < WIDTH && y < HEIGHT {
                        ...
                        // Compute top-left destination (scaled)
                        let wx = DISPLAY_POS_X + x * 2;
                        let wy = DISPLAY_POS_Y + y * 2;

                        unsafe {
                            if wx + 1 < WINDOW_WIDTH && wy + 1 < WINDOW_HEIGHT {
                                let base = wy * WINDOW_WIDTH + wx;
                                WINDOW_BUFFER[base] = pixel;
                                WINDOW_BUFFER[base + 1] = pixel;
                                WINDOW_BUFFER[base + WINDOW_WIDTH] = pixel;
                                WINDOW_BUFFER[base + WINDOW_WIDTH + 1] = pixel;
                            }
                        }
