...
const REGS_POS_X: usize = 136;
const REGS_POS_Y: usize = 720;
...
fn display_terminal(_window: &mut Window) {
    ...
    unsafe {
        ...
        for row in 0..TERMINAL_HEIGHT {
            ...
            draw_text(x, y, line.trim_end(), COL_GREEN, COL_CARD);
        }
...
fn put_px(x: usize, y: usize, rgb: u32) {
    let max_size = WINDOW_WIDTH * WINDOW_HEIGHT;
...
fn draw_text(mut x: usize, y: usize, s: &str, rgb: u32, blanking: u32) {
    // Clear background
    let text_width = s.len() * 6; // 5px + 1px gap
    fill_rect(x, y, text_width, 8, blanking);
    for ch in s.chars() {
...
fn draw_hex_u8(x: usize, y: usize, v: u8) {
    draw_text(x, y, &format!("{:02X}", v), COL_TEXT, COL_CARD);
}

fn draw_hex_u32(x: usize, y: usize, v: u32) {
    draw_text(x, y, &format!("{:08X}", v), COL_TEXT, COL_CARD);
}
