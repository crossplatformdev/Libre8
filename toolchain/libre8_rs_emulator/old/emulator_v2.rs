...
const REGS_POS_X: usize = 124;
const REGS_POS_Y: usize = 708;
...
fn display_flags(_window: &mut Window) {
    unsafe {
        ...
        for (i, (name, value)) in flags.iter().enumerate() {
            ...
            draw_text(x, y, name, 0xFFFFFF);
            x += 48;
            if *value {
                draw_text(x, y, "1", 0x00FF00);
            } else {
                draw_text(x, y, "0", 0xFF0000);
            }
        }
    }
}
...
fn display_registers(_window: &mut Window) {
    unsafe {
        ...
        draw_text(REGS_POS_X+5, REGS_POS_Y+5, "REGISTERS", 0xFFFFFF);
        let registers = [("A", REG_A), ("B", REG_B), ("C", REG_C), ("D", REG_D)];

        draw_text(REGS_POS_X+5, REGS_POS_Y+45, "HEX ", 0xFFFFFF);
        draw_text(REGS_POS_X+5, REGS_POS_Y+65, "DEC ", 0xFFFFFF);
        draw_text(REGS_POS_X+5, REGS_POS_Y+85, "CHR ", 0xFFFFFF);
        ...
        for (i, (name, value)) in registers.iter().enumerate() {
            ...
            draw_text(x, y, &value_str, 0x00FFFF);
            // Decimal
...
fn display_opcode(_window: &mut Window, opcode: u8) {
    unsafe {
        ...
        draw_text(OPCODE_POS_X + (6 * 22), OPCODE_POS_Y, &opcode_str, 0xFFFF00);
    }
}
...
fn display_pc(_window: &mut Window, pc: usize) {
    unsafe {
        ...
        draw_text(OPADDR_POS_X + (6 * 19), OPADDR_POS_Y, &pc_str, 0xFFFF00);
    }
}
    ...
    unsafe {
        ...
        for (i, value) in STACK.iter().rev().enumerate() {
            ...
            if Some(value) == STACK.last() {
                draw_text(x, y, &value_str, 0xFF0000);
            } else {
                if i == 0 {
                    draw_text(x, y, &value_str, 0xFFFF00);
                } else if i % 2 == 0 {
                    draw_text(x, y, &value_str, 0x0FFF00);
                } else {
                    draw_text(x, y, &value_str, 0xF0FF00);
                }
            }
        }
    }
...
fn display_terminal(_window: &mut Window) {
    ...
    unsafe {
        ...
        for row in 0..TERMINAL_HEIGHT {
            ...
            let line: String = TERMINAL_BUFFER[base..base + TERMINAL_WIDTH]
                .iter()
                .collect();
    ...
    for i in 0..16 {
        ...
        for j in 0..16 {
            ...
            if j == 0 {
                ...
                draw_text(mem_display_start_x, y, &addr_str, 0xFF0000);
            }

            let value_str = format!("{:02X}", memory[addr]);
            draw_text(x + 128, y, &value_str, 0x00FF00);
        }
    }
...
fn display_vram(_window: &mut Window, vram: &[u8], offset: usize) {
    ...
    for i in 0..16 {
        ...
        for j in 0..16 {
            ...
            if j == 0 {
                let addr_str = format!("{:04X}:", addr);
                draw_text(vram_display_start_x, y, &addr_str, 0xFF0000);
            }

            let value = format!("{:02X}", vram[addr]);
            draw_text(x + 128, y, &value, 0x00FF00);
        }
    }
}
...
fn draw_hex_u8(x: usize, y: usize, v: u8) {
    draw_text(x, y, &format!("{:02X}", v), COL_TEXT);
}

fn draw_hex_u32(x: usize, y: usize, v: u32) {
    draw_text(x, y, &format!("{:08X}", v), COL_TEXT);
}
...
fn refresh(window: &mut Window, bytes: &[u8], opcode: u8, addr: usize, x: usize, y: usize, VIDEO_BUFFER: &[u8]) {
    unsafe {
        ...
        display_memory(window, bytes, addr);
        display_vram(
            window,
            VIDEO_BUFFER,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while addr < num_bytes {
            ...
            POKE => {
                ...
                for i in 1..64 {
                    ...
                    if x < WIDTH && y < HEIGHT {
                        ...
                        let window_idx = (y * 2 * WINDOW_WIDTH) + (x * 2) + (DISPLAY_POS_Y * WINDOW_WIDTH) + DISPLAY_POS_X;
                        unsafe {
                            // Scale pixel by 2x
                            WINDOW_BUFFER[window_idx] = pixel;
                            WINDOW_BUFFER[window_idx + 1] = pixel;
                            WINDOW_BUFFER[window_idx + WINDOW_WIDTH] = pixel;
                            WINDOW_BUFFER[window_idx + WINDOW_WIDTH + 1] = pixel;
                        }
        ...
        if opcode == POKE {
            if addr % (WINDOW_WIDTH * WINDOW_HEIGHT) == 0 {               
