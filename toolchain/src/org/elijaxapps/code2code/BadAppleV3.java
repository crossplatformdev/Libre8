...
public class BadAppleV3 {
    ...
    public static void main(String[] args) {
        try {
            ...
            char[][][] bytes = new char[(1834-0)][32][144];
            for(int i = 0; i < 1834 - 0; i+=1){
                String ii = "" + (i + 0);
                int counter = 0;
                    
                frames[i] = readTextFile("frames/resized-__" + ii + ".txt");
                System.out.println(frames[i].length());
                for(int y = 0; y < 32; y++){
                    writer.print("POKE ");
                    for (int x = 0; x <= 72*2; x++) { 
                        char ch = frames[i].charAt(counter);
                        writer.print(ch);
                        //System.out.print(ch);
                        counter += 1;  
                    }
                ...
                //assert counter == frames[i].length() : "Counter is " + counter;
