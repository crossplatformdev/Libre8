...
public class CompilerV8 {
	...
	static long dataAddress = 0x00fffdffL;
			...
			case "__asm":
				...
				if (openBrace(brace)) {
					...
					while(!closeBrace(token)){
						...
						String asmLine = "";
						//token = nextToken();
						while(!newLine(token) && !closeBrace(token)){
							asmLine += token + " ";
							token = nextToken();
						}
						if(asmLine.trim().length() > 0){
							if(funName.toLowerCase().equals("main")){
								main.add("\n;;;;;;; main ;;;;;;;\n");
								main.add(".__MAIN__\n");					
		...
		if(!code.contains(header)){
			...
			while(!closeParenthesis(token)){
				...
				if(type(token)){
					...
					if(argNames.contains(argName) || variableNames.contains(argName)){
						argName = "_"+argName;
					}
					args += argName + " ";
					code.add("POPX\n");
